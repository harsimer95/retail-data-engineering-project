...
spark = SparkSession.builder \
    .master("local[*]") \
    .appName("Process Incoming Sales File") \
    .getOrCreate()

# Extract

file_path = "data/incoming/sales_2026-06-24.csv"
...
valid_output_path = f"data/processed/{folder_name}"
rejected_output_path = f"data/rejected/{folder_name}"

valid_df.write.mode("overwrite").parquet(valid_output_path)
rejected_df.write.mode("overwrite").parquet(rejected_output_path)

print("Valid processed data saved to:", valid_output_path)
